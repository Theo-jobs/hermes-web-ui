...
import { ChatContextCompressor, countTokens, estimateMessagesTokens, SUMMARY_PREFIX } from '../../lib/context-compressor'
...
async function convertInputForHermes(input: string | ContentBlock[]): Promise<string | any[]> {
  ...
  for (const block of input) {
    ...
    if (block.type === 'image') {
      try {
        const buf = await readFile(block.path)
        ...
        logger.warn(err, '[chat-run-socket] failed to inline image attachment %s', block.path)
        parts.push({ type: 'text', text: `[Image attachment unavailable: ${block.name || block.path}]` })
      }
      continue
    }

    if (block.type === 'file') {
      parts.push({ type: 'text', text: `[File: ${block.name || block.path}] ${block.path}` })
    }
  }
...
export function resolveSessionBoundRunConfig(
  ...
  return {
    // A draft session can be created before the first run (for example by
    // setting workspace metadata) with the then-active/default profile. When
    // the user selects a gateway for that empty draft, the run request is the
    // authoritative binding for the first message. Once messages exist, keep
    // the persisted session profile stable for the conversation.
...
export class ChatRunSocket {
  ...
  private onConnection(socket: Socket) {
    ...
    socket.on('run', async (data: {
      ...
      const requestedProfile = data.profile || profile
      if (data.session_id) {
        const state = this.getOrCreateSession(data.session_id)
        if (state.isWorking) {
          const bound = this.resolveBoundSession(data.session_id, requestedProfile, data.model)
          state.queue.push({
      ...
      await this.handleRun(socket, data, requestedProfile)
    })
  ...
  private async loadSessionStateFromDb(sid: string): Promise<SessionState> {
    try {
      ...
      return {
        ...
        isWorking: false,
        events: [],
        profile: (detail as any)?.session?.profile || (detail as any)?.profile,
        inputTokens,
        outputTokens,
        queue: [],
      }
  ...
  private async handleRun(
    socket: Socket,
    data: { input: string | ContentBlock[]; session_id?: string; model?: string; profile?: string; instructions?: string },
    profile: string,
    skipUserMessage = false,
  ) {
    const { input, session_id, instructions } = data
    const bound = this.resolveBoundSession(session_id, profile, data.model)
    const effectiveProfile = bound.profile
    const effectiveModel = bound.model
    const upstream = this.gatewayManager.getUpstream(effectiveProfile).replace(/\/$/, '')
    const apiKey = this.gatewayManager.getApiKeyForUpstream(effectiveProfile) || undefined
    ...
    if (session_id) {
      ...
      const existingSession = session_id ? getSession(session_id) : null
      if (existingSession && existingSession.message_count === 0 && existingSession.profile !== effectiveProfile) {
        updateSessionProfile(session_id, effectiveProfile)
      }
      ...
      if (!skipUserMessage) {
        ...
        if (!getSession(session_id)) {
          ...
          createSession({ id: session_id, profile: effectiveProfile, model: effectiveModel, title: preview })
        }
    ...
    try {
      ...
      const upstreamInput = await convertInputForHermes(input)
      const body: Record<string, any> = { input: upstreamInput }
      ...
      if (session_id) {
        try {
          ...
          if (detail?.messages?.length) {
            ...
            const contextLength = getModelContextLength(effectiveProfile)
            const triggerTokens = Math.floor(contextLength * 0.7)
            ...
            if (snapshot) {
              ...
              if (totalTokens <= triggerTokens && newMessages.length <= 150) {
                ...
                const afterTokenEstimate = estimateMessagesTokens(history)
                const completedPayload = {
                  ...
                  afterTokens: afterTokenEstimate,
                  skippedReason: 'existing_snapshot_under_threshold',
                  summaryTokens: countTokens(SUMMARY_PREFIX + snapshot.summary),
                  verbatimCount: newMessages.length,
                  compressedStartIndex: snapshot.lastMessageIndex,
                }
                this.replaceState(session_id, 'compression.completed', completedPayload)
                emit('compression.completed', completedPayload)
              } else {
                ...
                try {
                  ...
                  const afterTokenEstimate = estimateMessagesTokens(result.messages)
                  ...
                  this.replaceState(session_id, 'compression.completed', {
                    ...
                    skippedReason: result.meta.skippedReason,
                  ...
                  logger.info('[context-compress] AFTER  session=%s: %d messages, ~%d tokens (was %d)', session_id, result.messages.length, afterTokenEstimate, totalTokens)
      ...
      // Keep the normalized upstream input built by convertInputForHermes().
      // It preserves image attachments as OpenAI-compatible image_url parts
      // with data:image/... URLs. Converting ContentBlock[] back to plain
      // text here would drop the actual image payload, so WebUI uploads would
      // only reach Hermes as "[Image: /path]" text.
...
// Flush in-memory assistant text to DB before aborting the stream.
  ...
  private resolveBoundSession(
    sessionId: string | undefined,
    requestedProfile: string | undefined,
    requestedModel?: string,
  ): { profile: string; model?: string } {
    const fallbackProfile = requestedProfile || 'default'
    if (!sessionId) return { profile: fallbackProfile, model: requestedModel }

    const row = getSession(sessionId)
    const state = this.sessionMap.get(sessionId)
    return resolveSessionBoundRunConfig(row, state?.profile, fallbackProfile, requestedModel)
  }
