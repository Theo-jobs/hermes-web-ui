...
export class ChatRunSocket {
  ...
  private onConnection(socket: Socket) {
    ...
    const profileExists = (profile: string) => isKnownRunProfile(profile)
    ...
    socket.on('run', async (data: {
      ...
      let bound: ReturnType<typeof resolveSessionBoundRunConfig>
      try {
        const requestedProfile = resolveRunProfile(data.session_id, data.profile)
        bound = data.session_id
