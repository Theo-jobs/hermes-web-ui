...
<p align="center">
  <img src="./packages/client/src/assets/image1.png" alt="Hermes Web UI 演示" width="680"/>
</p>

<p align="center">
  <img src="./packages/client/src/assets/image2.png" alt="Hermes Web UI 演示" width="680"/>
</p>
...
<p align="center">
  ...
  <a href="./LICENSE"><img src="https://img.shields.io/npm/l/hermes-web-ui?style=flat-square" alt="许可证"/></a>
  <a href="https://github.com/Theo-jobs/hermes-web-ui"><img src="https://img.shields.io/github/stars/Theo-jobs/hermes-web-ui?style=flat-square" alt="Star"/></a>
</p>
...
本分支感谢并继续跟随上游 [`EKKOLearnAI/hermes-web-ui`](https://github.com/EKKOLearnAI/hermes-web-ui) 与 [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent)。Web UI 的基础能力、Hermes Agent 集成、界面框架、网关能力和大部分通用功能均来自上游项目；Theo / EKKO 分支是在此基础上做本地使用场景适配。

本次已合并上游 `v0.5.25`，主要包含：

- 群聊房间新增重置和克隆操作。
- Web UI 状态目录支持配置，方便自定义部署路径。
- 语音设置新增 MiMo TTS Provider。
- 自定义 Provider 模型列表改由后端代理请求，避免浏览器 CORS 限制。
- 修复 Bridge 会话的工具授权流程。
- 移除 Bridge 强制注入的 CLI 平台提示，避免覆盖用户自定义媒体和文件输出规则。
- 用户消息历史支持正确展示 base64 图片内容。
- 新增 Playwright 浏览器测试、聊天流式契约测试、Provider 模型测试和覆盖率基线。
...
4. 解决冲突后运行测试和构建。
5. 推送到 `theo/master`，并重启本地服务确认 `/health` 返回新版本。
...
```bash
bash <(curl -fsSL https://cdn.jsdelivr.net/gh/Theo-jobs/hermes-web-ui@master/scripts/setup.sh)
```
...
```bash
git clone -b master https://github.com/Theo-jobs/hermes-web-ui.git
cd hermes-web-ui
npm install
npm run dev
```
